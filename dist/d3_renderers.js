...
callWithJQueryAndD3(($, d3) => $.pivotUtilities.d3_renderers = {
  Treemap(pivotData, opts) {
    ...
    const treemap = d3.layout.treemap().size([width, height]).sticky(true).value(d => d.size);
    d3.select(result[0]).append('div').style('position', 'relative').style('width', width + 'px').style('height', height + 'px').datum(tree).selectAll('.node').data(treemap.padding([15, 0, 0, 0]).value(d => d.value).nodes).enter().append('div').attr('class', 'node').style('background', function (d) {
      if (d.children != null) {
        return 'lightgrey';
      } else {
        return color(d.name);
      }
    }).text(d => d.name).call(function () {
      this.style('left', d => d.x + 'px').style('top', d => d.y + 'px').style('width', d => Math.max(0, d.dx - 1) + 'px').style('height', d => Math.max(0, d.dy - 1) + 'px');
    });
    return result;
  }
});
